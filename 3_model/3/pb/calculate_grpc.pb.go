...
type CalculateServiceClient interface {
	Sum(ctx context.Context, opts ...grpc.CallOption) (CalculateService_SumClient, error)
}
...
func (c *calculateServiceClient) Sum(ctx context.Context, opts ...grpc.CallOption) (CalculateService_SumClient, error) {
	stream, err := c.cc.NewStream(ctx, &CalculateService_ServiceDesc.Streams[0], "/CalculateService/Sum", opts...)
	if err != nil {
		return nil, err
	}
	x := &calculateServiceSumClient{stream}
	return x, nil
}

type CalculateService_SumClient interface {
	Send(*SumRequest) error
	CloseAndRecv() (*SumResponse, error)
	grpc.ClientStream
}

type calculateServiceSumClient struct {
	grpc.ClientStream
}

func (x *calculateServiceSumClient) Send(m *SumRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *calculateServiceSumClient) CloseAndRecv() (*SumResponse, error) {
...
type CalculateServiceServer interface {
	Sum(CalculateService_SumServer) error
...
func (UnimplementedCalculateServiceServer) Sum(CalculateService_SumServer) error {
...
func _CalculateService_Sum_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(CalculateServiceServer).Sum(&calculateServiceSumServer{stream})
}

type CalculateService_SumServer interface {
	SendAndClose(*SumResponse) error
	Recv() (*SumRequest, error)
	grpc.ServerStream
}

type calculateServiceSumServer struct {
	grpc.ServerStream
}

func (x *calculateServiceSumServer) SendAndClose(m *SumResponse) error {
	return x.ServerStream.SendMsg(m)
}

func (x *calculateServiceSumServer) Recv() (*SumRequest, error) {
	m := new(SumRequest)
